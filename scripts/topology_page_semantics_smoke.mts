...
assert.match(topoPageCode, /tabIndex=\{canvasMaximized \? -1 : undefined\}/, 'Focused canvas dialog should be programmatically focusable')
assert.match(topoPageCode, /previousCanvasFocusRef/, 'Focused canvas should restore focus on exit')
assert.match(topoPageCode, /canvasCardRef\.current\?\.focus\(\{ preventScroll: true \}\)/, 'Focused canvas should move keyboard focus into the dialog')
...
// P4-2 redesign: dense control + lower topology panels are now always-open fixed cards
// (fixed 3-panel layout + right tabs) instead of +/- <details> accordions.
assert.doesNotMatch(topoPageCode, /<details className="panel-card collapsible-panel"/, 'P4-2: topology panels should not use collapsible <details> accordions')
assert.match(topoPageCode, /topo-static-panel/, 'P4-2: dense topology panels should be always-open fixed cards')
assert.match(topoPageCode, /topology-control-panel/, 'Control panels should remain test-addressable by class')
assert.match(topoPageCode, /<span className="mini-status">\{compareLayoutStatus\}<\/span>/, 'Primary controls heading should still surface the active layout state')
assert.match(topoPageCode, /controls-layout topo-static-grid/, 'Controls should use the always-open fixed-grid treatment')
assert.match(topoPageCode, /three-panels topo-static-grid/, 'Lower topology panels should sit in the always-open fixed grid')
