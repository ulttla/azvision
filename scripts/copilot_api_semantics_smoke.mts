...
const backendConfigCode = readFileSync(path.join(repoRoot, 'backend/app/core/config.py'), 'utf8')
const envExampleCode = readFileSync(path.join(repoRoot, '.env.example'), 'utf8')
...
assert.match(backendConfigCode, /openrouter_app_title/, 'Settings should expose optional OpenRouter app title config')
assert.match(envExampleCode, /OPENROUTER_HTTP_REFERER=/, '.env.example should document optional OpenRouter referer config')
assert.match(envExampleCode, /OPENROUTER_APP_TITLE=AzVision/, '.env.example should document optional OpenRouter app title config')
