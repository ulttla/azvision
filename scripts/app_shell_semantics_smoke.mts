...
assert.match(appCode, /data-first-run=\{isFirstRun \? 'true' : 'false'\}/, 'Public beta onboarding should expose first-run state for browserless and UI tests')
assert.match(appCode, /isFirstRun \? t\('publicBeta\.firstRun\.title'\) : t\('publicBeta\.title'\)/, 'Public beta onboarding should render conditional first-run title copy')
