...
- backend
  ...
  - snapshot compare first-pass endpoint 구현 완료. 기본 `/snapshots/compare`는 saved view-state metadata 기준으로 count/scope/compare_refs 차이를 반환한다. raw topology archival diff는 `snapshot_topology_archives` 저장소, `POST /snapshots/{id}/topology-archive`, `POST /snapshots/compare/topology`, auto-archive, bounded UI drilldown/markdown export까지 구현됐다. retention은 `docs/RETENTION_POLICY.md` 기준으로 health signal/design까지만 완료됐으며 실제 prune/delete는 별도 승인 대상이다
  ...
  - `/chat` copilot은 read-only LLM provider MVP skeleton까지 확장됨. `GET /copilot/providers`, `POST /copilot/chat`, 기존 `POST /workspaces/{workspace_id}/chat` 호환 route를 제공하며, Ollama/Ollama Cloud와 OpenRouter backend-only provider 설정을 지원한다. OpenRouter는 optional app attribution headers(`OPENROUTER_HTTP_REFERER`, `OPENROUTER_APP_TITLE`)를 backend-only 요청에 붙일 수 있고, app title은 `X-OpenRouter-Title`과 legacy `X-Title`로 함께 전송한다. provider 미설정, disabled, provider 오류 시 secret 노출 없이 rule-based fallback으로 응답한다.
...
- 참고
  ...
  - 다음 개발 우선순위는 `docs/PERSONAL_USE_READINESS_PLAN.md` 의 `Next development priorities after v0.9` 섹션을 기준으로 한다. read-only LLM Copilot MVP는 provider/config/context/selector skeleton과 local-only hardening baseline이 들어왔고, 개인용 v0.9 판단은 `scripts/personal_use_acceptance.sh` 통과 여부를 우선 기준으로 본다. 다음 Copilot slice는 opt-in live UI smoke 유지, 필요한 경우 더 깊은 `CopilotPanel` UI coverage, hosted/OpenRouter polish이다. Architecture View polish는 stable path에 들어왔고, export smoke/test 유지, Azure Arc/hybrid 확장, productization 재진입은 후속 순서로 관리한다.
