...
- `OpenRouterCopilotProvider`: OpenRouter API-key path for local or hosted deployments.
  - Optional non-secret app attribution headers can be set with `OPENROUTER_HTTP_REFERER` and `OPENROUTER_APP_TITLE`; app title is sent as `X-OpenRouter-Title` plus legacy `X-Title` for compatibility.
