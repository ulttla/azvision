...
| G3 | API protection and audit trail | partial | `X-Request-ID`, safe request logging, `build_rate_limited_response()`, `InMemoryRateLimiter` behind `AZVISION_RATE_LIMIT_ENABLED`, OIDC session-specific limiter bucket, shared enforcement runbook, indexed `audit_events` table, `record_audit_event()`, auth/session/workspace/credential/export/snapshot audit tests, `docs/RATE_LIMIT_AUDIT_PLAN.md`, `backend/tests/test_rate_limit_middleware.py` | Enable shared storage/gateway enforcement for multi-instance public traffic and finish audit coverage for remaining public-abuse-sensitive routes |
