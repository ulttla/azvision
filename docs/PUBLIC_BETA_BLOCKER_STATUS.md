...
| G3 | API protection and audit trail | partial | `X-Request-ID`, safe request logging, `build_rate_limited_response()`, indexed `audit_events` table, `record_audit_event()`, auth/session/workspace/credential/export/snapshot audit tests, `docs/RATE_LIMIT_AUDIT_PLAN.md` | Implement actual limiter and finish audit coverage for remaining public-abuse-sensitive routes |
