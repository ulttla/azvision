...
- `GET /workspaces/{workspace_id}/path-analysis`
  - query: `source_resource_id`, `destination_resource_id`, optional `subscription_id`, `resource_group_name`, `resource_limit`, `protocol`, `source_address_prefix`, `destination_address_prefix`, `source_port`, `destination_port`
  ...
  - `path_candidates[]`에는 hop list와 함께 `peering_hop_count` 및 `is_forwarded_traffic`이 포함될 수 있다. `peering_hop_count=0`은 intra-VNet, `1`은 direct peering, `2+`는 forwarded/transitive peering candidate를 뜻한다. `is_forwarded_traffic`은 intra-VNet이면 `null`, direct peering이면 `false`, forwarded/transitive peering이면 `true`다. hop 응답에는 VNet peering crossing target 여부를 표시하는 `is_peering_boundary`가 항상 포함된다.
  - 현재 MVP는 Azure explicit topology edge 중 traffic-carrying `connects_to` 관계만 path tracing에 사용하고, NSG `secures` / route table `routes` 관계는 hop classification의 control data로만 사용한다. VNet peering edge는 양쪽 VNet의 reciprocal peering evidence가 있고 양쪽 `peeringState`가 `Connected`일 때만 traversal에 사용한다. Peering 방향에 `allowVirtualNetworkAccess=false`가 명시되면 해당 방향 traversal은 후보에서 제외한다. Direct peering traffic은 `allowForwardedTraffic` 없이도 후보가 될 수 있지만, 2개 이상의 peering edge를 건너는 forwarded/transitive traversal은 지나간 모든 peering 방향에 `allowForwardedTraffic=true`가 명시된 경우에만 후보로 보며, missing/false는 path 미발견(`unknown`) 쪽으로 보수 처리한다.
  - NSG rule은 source 쪽 outbound와 destination 쪽 inbound를 모두 평가한다. NIC와 subnet에 NSG가 함께 연결된 경우 둘 다 effective NSG evidence로 반영하며, 둘 중 하나라도 `blocked`면 path를 `blocked`로 본다. `defaultSecurityRules`가 inventory payload에 없거나 일부만 있으면 Azure 기본 NSG rule의 누락분을 내부적으로 보강한다.
  - optional `protocol` / `source_address_prefix` / `destination_address_prefix` / `source_port` / `destination_port`가 들어오면 rule의 protocol, source/destination prefix, source/destination port/range를 보수적으로 필터링한다. `VirtualNetwork` service tag는 broad RFC1918 전체가 아니라 NSG가 걸린 hop의 owning VNet address space와 reciprocal connected peered VNet address space 기준으로 먼저 해석한다. hop 응답에는 기존 `nsg_verdict` 외에 `nsg_name`, 실제 winning rule 기준의 `nsg_rule_name`, `nsg_direction`, `nsg_outbound_verdict`, `nsg_outbound_name`, `nsg_outbound_rule_name`가 포함될 수 있다.
  - Azure service tag는 `Internet`, `AzureLoadBalancer`, `Storage` 등 주요 tag에 대해 static approximation으로 해석한다. 단 `VirtualNetwork`는 path context가 있을 때 owning VNet/connected peering scope를 우선 사용한다. 모르는 tag나 해석 불가능한 prefix는 `unknown` 쪽으로 남긴다.
  - route table은 `properties.routes[]`를 읽고 longest-prefix-match 후보를 먼저 고른 뒤 `nextHopType=None` black-hole route를 `blocked`로 본다. `Internet`, `VnetLocal`, `VirtualNetwork`는 단순 허용 후보로 보지만, `VirtualAppliance`, `VirtualNetworkGateway`는 방화벽/게이트웨이 내부 상태를 모르면 확정할 수 없어 `unknown`으로 둔다. CIDR 포함 관계는 Python stdlib `ipaddress` 기반으로 해석하며, route table address prefix의 주요 Azure service tag도 같은 방식으로 해석한다. hop 응답의 `route_table_name` / `route_name` / `route_next_hop_type` / `route_next_hop_ip`은 verdict를 실제로 결정한 route evidence를 가리킨다.
  - source/destination 미발견, path 미발견, NSG/route data 부족 또는 해석 불확실성은 `unknown`으로 반환한다. 즉, 데이터가 없을 때 `allowed`로 가정하지 않는다.
  - 이 분석은 inventory payload에 있는 정적 topology/NSG/route/peering flag evidence 기준의 보수적 추정이며, BGP propagated routes, gateway transit, NVA/firewall 내부 forwarding, live packet connectivity 검증은 아직 포함하지 않는다.
...
- non-2xx backend 응답은 기본적으로 JSON body `{ ok: false, status, message }` shape를 사용한다
  ...
  - `500` (unexpected): debug/dev에서는 세부 오류를 유지할 수 있으나, debug-off production에서는 `{ ok: false, status: "http-500", message: "Internal server error" }`처럼 내부 detail을 숨긴다.
