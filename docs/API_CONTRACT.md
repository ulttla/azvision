...
- `GET /workspaces/{workspace_id}/path-analysis`
  ...
  - 현재 MVP는 Azure explicit topology edge 중 traffic-carrying `connects_to` 관계만 path tracing에 사용하고, NSG `secures` / route table `routes` 관계는 hop classification의 control data로만 사용한다. VNet peering edge는 양쪽 VNet의 reciprocal peering evidence가 모두 있을 때만 traversal에 사용한다.
  ...
  - optional `protocol` / `source_address_prefix` / `destination_address_prefix` / `source_port` / `destination_port`가 들어오면 rule의 protocol, source/destination prefix, source/destination port/range를 보수적으로 필터링한다. hop 응답에는 기존 `nsg_verdict` 외에 `nsg_name`, 실제 winning rule 기준의 `nsg_rule_name`, `nsg_direction`, `nsg_outbound_verdict`, `nsg_outbound_name`, `nsg_outbound_rule_name`가 포함될 수 있다.
