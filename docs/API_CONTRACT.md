...
- `GET /workspaces/{workspace_id}/path-analysis`
  ...
  - optional `protocol` / `source_address_prefix` / `destination_address_prefix` / `destination_port`가 들어오면 rule의 protocol, source/destination prefix, destination port/range를 보수적으로 필터링한다. hop 응답에는 기존 `nsg_verdict` 외에 `nsg_name`, 실제 winning rule 기준의 `nsg_rule_name`, `nsg_direction`, `nsg_outbound_verdict`, `nsg_outbound_name`, `nsg_outbound_rule_name`가 포함될 수 있다.
