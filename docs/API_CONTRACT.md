...
- `GET /workspaces/{workspace_id}/path-analysis`
  ...
  - 현재 MVP는 Azure explicit topology edge 중 traffic-carrying `connects_to` 관계만 path tracing에 사용하고, NSG `secures` / route table `routes` 관계는 hop classification의 control data로만 사용한다. VNet peering edge는 양쪽 VNet의 reciprocal peering evidence가 있고 양쪽 `peeringState`가 `Connected`일 때만 traversal에 사용한다.
