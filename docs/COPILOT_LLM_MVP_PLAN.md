...
# Optional non-secret OpenRouter app attribution headers.
...
- Optional live LLM UI smoke is available as `scripts/copilot_live_ui_smoke.mjs`; it is opt-in only via `AZVISION_LIVE_COPILOT_SMOKE=1` so normal smoke runs do not depend on LLM latency or provider availability. When explicitly enabled, it verifies the shared Copilot provider storage value is restored before submit, then checks that the rendered answer omits obvious provider secret/config markers.
...
- Backend pytest collection is currently 378 tests.
- `npm --prefix frontend run build` passes.
