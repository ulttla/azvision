...
This is not yet full public beta auth. It is a route-level isolation, session lookup, local dev-session, OIDC entrypoint/verifier/mapping contract, credential ownership, and initial audit-write contract slice. Public beta still needs account lifecycle UX, provider-specific configuration, shared rate-limit enforcement, and broader audit event coverage before exposure. The session persistence path itself is already tested and reused by the OIDC entrypoint contract.
