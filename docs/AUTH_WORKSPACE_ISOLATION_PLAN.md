...
- `/auth/config-check` now reports OIDC readiness with non-secret booleans and counts only: login enabled, issuer/audience/JWKS presence, workspace map presence/validity, mapped user count, and grant count. It also reports account lifecycle readiness with `dev_session_enabled`, `oidc_login_enabled`, `account_management_enabled`, and `public_routes_exposure_gated`, plus shared limiter readiness booleans/counts. `public_routes_exposure_gated` is a conservative UI aggregate over dev-session/OIDC/account-management flags, not a substitute for hosted exposure proof. It does not echo issuer, audience, JWKS URL, mapped email, workspace IDs, provider names, or secrets.
