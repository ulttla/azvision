...
This is not yet full public beta auth. It is a route-level isolation, session lookup, local dev-session, OIDC entrypoint/verifier/mapping contract, credential ownership, OIDC readiness visibility, and audit-write contract slice. Public beta still needs account lifecycle UX, provider-specific values to be selected/entered for the target environment, and shared rate-limit enforcement evidence before exposure. The session persistence path itself is already tested and reused by the OIDC entrypoint contract.
